...
with col_main:
    ...
    # =========================================================================
        ...
        with sub_tabs_plan[1]:
                ...
                with st.container(border=True):
                    ...
                    if selected_ticker in current_db["data"]:
                        ...
                        if "BDC" in asset_class:
                            # 🔴 하드코딩이었던 40.0, 6.0, 1.5를 사용자가 입력한 파라미터 변수로 교체
            ...
            # 🔴 신규 융합: "가짜 난수(Proxy)" 대신 "진짜 API 데이터(Real Data)" 우선 호출 로직 적용
            ...
            try:
                with st.spinner("실제 글로벌 기초자산 주가 데이터를 실시간 수집 중입니다... (API 호출)"):
                    spy_df = fdr.DataReader('SPY', start_dt, end_dt) # 벤치마크 S&P 500 ETF
                    ...
                    for idx, tkr in enumerate(tkrs):
                        ...
                        if w > 0:
                            ...
                            if len(df_tkr) > 200: # 최소 1년 이상의 과거 데이터가 있는 종목만 채택
            ...
            # 데이터가 성공적으로 불러와졌다면 실제 수익률 계산, 아니면 기존 난수(Proxy)로 Fallback
        ...
        with sub_tabs_plan[2]:
            ...
            # 100% 원본 복구 및 신규 융합: P&L (투입변수 유동화 적용)
            with st.container(border=True):
