...
# -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
        ...
        prompt_tabs = st.tabs(["📊 1. 주간 모니터링 5-Step 체인 프롬프트", "🌟 2. 상품 기획 RAG 마스터 프롬프트"])
...
특히, T-0, T-1, T-2 산점도 수익률 반응도 데이터를 근거로 이번 주 리테일 자금이 '과거 수익률(T-1, T-2)'을 쫓아 들어왔는지, '당일 테마(T-0)'에 직각적으로 반응했는지 자금 유입의 성격을 진단하시오."""
            ...
            # [수정] 눈에 띄는 시각 자료 첨부 안내 (warning 박스 사용)
            st.warning("💡 **[필수 시각자료 첨부]** 완벽한 인과관계 분석을 위해, 1번 탭의 **'마케팅 임팩트 분석기(전체 순매수 추이 궤적)' 차트 캡처 이미지**를 이 프롬프트와 함께 AI 대화창에 꼭 업로드하세요!")
...
2. 기관 타겟 (세일즈/영업): [{tgt}]의 [{cmp}] 대비 강점(AUM 규모, 보수율 등)을 엮어, 법인영업팀 및 PB들이 거액 자산가/기관에게 던질 수 있는 강력한 '원 라이너(1-liner) 세일즈 멘트'를 도출할 것."""
        ...
        # --- 상품 기획 프롬프트 (5-Step) 개편 ---
        with prompt_tabs[1]:
            st.markdown("#### [글로벌 대체자산 ETF 상품기획 프롬프트 - 5-Step 체인]")
            st.caption("고품질의 상세한 상품 기획서(5~6페이지 분량)를 도출하기 위해, 샌드박스의 데이터를 통합한 5단계 체인 프롬프트입니다. 순서대로 복사하여 AI(ChatGPT, Gemini 등)에 입력하세요.")
            ...
            if st.session_state.get('p_has_csv', False):
                ...
                csv_directive = f"첨부된 유니버스 엑셀(CSV) 데이터를 분석하여, 위 펀더멘털 필터링 룰(LTV {st.session_state.p_ltv}% 이하, FCF 마진 {st.session_state.p_fcf}% 이상)을 통과한 최종 편입 종목의 리스트를 기획서 포트폴리오 섹션에 표 형태로 출력할 것."
            else:
            ...
            p2_step1 = f"""너는 최고 수준의 자산운용사 ETF 상품개발(PD) 시니어 수석 매니저야. 첫 번째 작업으로 아래 데이터를 바탕으로 **[1. 기획 배경 및 글로벌 트렌드]** 파트를 아주 상세하게(약 1페이지 분량) 작성해 줘.

- 기획 의도: {user_idea}
- 시장 배경 (미국 메가트렌드): 현재 미국 시장에서는 '{st.session_state.get('selected_trend_label', '혁신 타겟 인컴')}' 테마가 강세로 자금을 강하게 흡수하고 있으나, 당사 라인업은 공백 상태임.
- 지시사항: 위 기획 의도와 최근 모니터링된 관련 정책/규제 완화 시그널을 근거로, 왜 지금 당장 이 상품을 선제적으로 기획하여 국내 시장에 출시해야 하는지 '출시 당위성 및 거시적 배경'을 논리적으로 작성할 것."""
            st.code(p2_step1, language="text")

            st.markdown("**📌 [Step 2: 지수 산출 방법론 및 유니버스]**")
            p2_step2 = f"""두 번째 작업으로 본 ETF의 뼈대가 될 **[2. 지수 산출 방법론 및 유니버스]** 파트를 상세히 작성해 줘.

- 기초자산 프록시: {st.session_state.p_proxy}
- 프록시 선정 논리: {st.session_state.p_proxy_reason}
- 펀더멘털 스크리닝: LTV(부채비율) {st.session_state.p_ltv}% 이하, 잉여현금흐름(FCF) 마진 {st.session_state.p_fcf}% 이상을 허들로 설정.
- 편입 후보 종목(샌드박스 연동): 시뮬레이션을 통해 1차 검증된 티커 리스트.
- 지시사항: {csv_directive} 이 스크리닝 룰과 편입 후보 종목들을 기반으로 지수 산출 방법론을 구체화하고, 가중치 배분([{st.session_state.p_weight}], 단일 종목 상한 {st.session_state.p_cap}%) 룰 및 S&P DJI 방식의 리밸런싱/이벤트 처리 원칙을 명시할 것."""
            st.code(p2_step2, language="text")

            st.markdown("**📌 [Step 3: 퀀트 성과 및 리스크 검증]**")
            # [수정] 스크린샷 첨부 눈에 띄게 강조
            st.warning("📸 **[핵심 차트 스크린샷 첨부]** 기관 투자자 설득을 위한 필수 근거입니다. 2번 탭의 **'하이브리드 시나리오 차트 (가상 합성지수 궤적)' 이미지**를 캡처하여 프롬프트와 함께 첨부해 주세요.")
            p2_step3 = f"""세 번째 작업으로 합성 지수의 실전적 **[3. 퀀트 퍼포먼스 및 리스크 검증]** 파트를 작성해 줘.

- 퀀트 백테스트 지표: 샤프비율 {st.session_state.p_sharpe}, MDD {st.session_state.p_mdd}%, S&P 500 상관계수 {st.session_state.p_corr}.
- 마찰 비용 할인: 위 수치는 LP 호가 스프레드 및 리밸런싱 슬리피지를 감안하여 [연 {st.session_state.get('p_lp_cost', 0.2)}%]의 보수적인 할인율이 차감된 수치임.
- 지시사항: 1) 첨부된 차트 이미지와 S&P 500과의 낮은 상관계수를 근거로 기존 전통자산 포트폴리오 편입 시 분산(헷지) 효과를 수학적으로 증명할 것. 2) 총수익률을 자본차익과 인컴수익으로 철저히 분해하여, 하락장 방어 논리를 어필할 것. 3) 과거 매크로 위기 국면([{st.session_state.p_scenario}]) 당시의 벤치마크 대비 방어력을 분석할 것. 4) 합성 지수의 복제 추적오차(Tracking Error) 리스크를 통제하기 위한 실제 운용 매니저 관점의 대응 전략을 추가할 것."""
            st.code(p2_step3, language="text")

            st.markdown("**📌 [Step 4: 상품 구조화 및 운용사 P&L 분석]**")
            # [수정] P&L 차트 스크린샷 첨부 강조
            st.warning("📸 **[P&L 차트 스크린샷 첨부]** 경영진 보고를 위한 필수 근거입니다. 2번 탭의 **'P&L 폭포수(Waterfall) 차트' 이미지**를 캡처하여 프롬프트와 함께 첨부해 주세요.")
            p2_step4 = f"""네 번째 작업으로, 실질적인 상품 런칭 및 재무 타당성을 다루는 **[4. 상품 구조화 및 운용사 비즈니스 모델]** 파트를 경영진 보고용으로 작성해 줘.

- 환율 전략: [{st.session_state.p_fx}] 
- 운용사 P&L 및 M/S 탈환: 타겟 경쟁사([{st.session_state.get('p_comp_ticker', '유사ETF')}])의 보수율({st.session_state.get('p_comp_ter', 0.5)}%) 대비 당사 보수율 경쟁력을 기반으로 한 AUM 뺏어오기(Switching) 전략 구체화. 첫해 타겟 AUM {st.session_state.p_aum}억 원 달성 시, 고정비/마케팅비를 차감한 운용사 예상 순수익은 {st.session_state.p_profit}억 원으로 추정됨.
- 지시사항: 1) 본 상품의 환율 전략과 인컴 속성을 고려할 때, 고객 세후 수익률에 유리한 연금/ISA 채널 타겟팅 전략을 세울 것. 2) 위 P&L 데이터와 첨부된 폭포수 차트 이미지를 근거로, 상품 런칭 시점의 초기 시딩(Seeding) 규모와 마케팅 비용 집행이 시장 점유율 확보 관점에서 왜 타당한 투자인지 강력하게 설득할 것."""
            st.code(p2_step4, language="text")

            st.markdown("**📌 [Step 5: 요약 보고서 및 리테일 세일즈 팩트시트 산출]**")
            # [수정] 기관/리테일 투트랙 세일즈 팩트시트로 프롬프트 내용 전면 개편
            p2_step5 = """마지막 작업이야. Step 1부터 Step 4까지 전개한 모든 퀀트 논리와 수치 데이터를 총망라하여, 다음 두 가지 실무 산출물을 각각 분리해서 최종 완성해 줘.

1. **[Executive Summary (임원 보고용 요약본)]**: 본부장 및 C-Level 임원진이 1분 안에 의사결정을 내릴 수 있도록 1페이지 분량으로 요약된 공문서. (출시 당위성, 핵심 퀀트 성과, BEP 달성 및 경쟁사 타겟팅 전략이 일목요연하게 정리되어야 함)
2. **[Two-Track Sales Factsheet (투트랙 세일즈 팩트시트)]**: 
   - **리테일/PB용 (1p):** 일반 고객을 사로잡을 직관적인 카피라이팅, 핵심 소구 포인트 3가지, 투자 위험도 및 세금(Tax) 혜택 활용법.
   - **기관/법인영업용 (1p):** 연기금 등 기관 투자자를 설득할 퀀트 데이터(추적오차 통제, S&P 500 대비 낮은 상관관계, 꼬리 위험 헷지 효과)와 LP 호가 스프레드 안정성 위주의 논리 전개.
