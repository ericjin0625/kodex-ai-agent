...
with col_main:
    ...
    if big_tab == "ETF 시장 모니터링":
        ...
        sub_tabs = st.tabs(["🏠 Home", "📊 Weekly Info", "📈 순매수 & 수익률", "📰 뉴스 & 트렌드", "💸 거래량 추이", "📺 마케팅 성과 검증(DiD/Lag)", "🗣️ 고객 UX", "🥧 ETF/AUM 현황"])
        ...
        with sub_tabs[5]:
            ...
            with st.container(border=True):
                ...
                with col_tgt:
                    target_etf = st.selectbox("🎯 Target (이벤트 대상):", ["KODEX 200", "KODEX 반도체", "KODEX 2차전지산업"], index=0)
                with col_comp:
                    comp_etf = st.selectbox("⚔️ Competitor (대조군):", ["TIGER 200", "TIGER 반도체", "RISE 코리아밸류업"], index=0)
                with col_period1:
                    event_name = st.selectbox("📣 분석 대상 마케팅 액션:", ["국민은행 KODEX 이벤트 (4/13~4/30)", "TIGER 반도체 유튜브 프로모션"], index=0)
                with col_period2:
                    baseline_period = st.slider("📉 통계 베이스라인 기간 (이벤트 전 N주):", 1, 8, 2, help="이벤트 발생 이전의 평균적인 순매수/검색량 추이를 정의합니다.")
            
            st.divider()

            # Mock Statistical Data Generation (Based on PDF)
            # In a real environment, this would calculate from `uploaded_excel` and `uploaded_dls`.
            mock_did_multiplier = 1.37
            mock_p_value = 0.0008
            mock_net_inflow = 13.23
            mock_brand_search_inc = 42.9
            
            st.session_state.stat_did_multiplier = mock_did_multiplier
            st.session_state.stat_p_value = mock_p_value
            st.session_state.stat_net_inflow = mock_net_inflow

            # [Step 2] 상단: 핵심 계량 지표 카드 (High-Level Metrics Dashboard)
            st.markdown("#### 💡 [Step 2] 핵심 계량 지표 (High-Level Metrics)")
            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
            with col_m1:
                st.metric("순수 설정효과 (신규 자금)", f"+{mock_net_inflow}억원", "시장효과 제거됨", delta_color="normal")
            with col_m2:
                st.metric("이중차분(DiD) 성과 배수", f"{mock_did_multiplier}배", f"대조군({comp_etf}) 대비", delta_color="normal")
            with col_m3:
                st.metric("브랜드 관심도 상승률", f"+{mock_brand_search_inc}%", "이벤트 전 대비", delta_color="normal")
            with col_m4:
                p_text = f"p < 0.001" if mock_p_value < 0.001 else f"p = {mock_p_value:.3f}"
                st.metric("통계적 유의성 (Welch's t-test)", p_text, "유의미함 (p < 0.05)", delta_color="normal")
            ...
            with c_chart1:
                with st.container(border=True):
                    st.markdown("**📈 이벤트 스터디 및 누적초과검색량(CAR) 팬 차트**")
                    # Mock Data for Fan Chart (CAR)
                    np.random.seed(42)
                    days = pd.date_range(start="2026-04-01", end="2026-05-15", freq='D')
                    base_car = np.concatenate([
                        np.random.normal(0, 0.5, 12).cumsum(),  # Pre-event
                        np.linspace(0, 8, 18) + np.random.normal(0, 0.5, 18), # Event (4/13~4/30)
                        np.linspace(8, 18, 15) + np.random.normal(0, 0.5, 15) # Post-event
                    ])
                    df_car = pd.DataFrame({'Date': days, 'CAR': base_car})
                    df_car['Upper'] = df_car['CAR'] + (np.arange(len(days)) * 0.1)
                    df_car['Lower'] = df_car['CAR'] - (np.arange(len(days)) * 0.1)

                    fig_car = go.Figure()
                    fig_car.add_trace(go.Scatter(x=df_car['Date'], y=df_car['Lower'], mode='lines', line=dict(width=0), showlegend=False))
                    fig_car.add_trace(go.Scatter(x=df_car['Date'], y=df_car['Upper'], mode='lines', fill='tonexty', fillcolor='rgba(77, 166, 255, 0.2)', line=dict(width=0), name='95% 신뢰구간'))
                    fig_car.add_trace(go.Scatter(x=df_car['Date'], y=df_car['CAR'], mode='lines+markers', line=dict(color='#4da6ff', width=3), name='누적초과관심도(CAR)'))
                    
                    # Event Period Shading
                    fig_car.add_vrect(x0="2026-04-13", x1="2026-04-30", fillcolor="rgba(255, 176, 77, 0.1)", layer="below", line_width=0, annotation_text="이벤트 기간", annotation_position="top left")
                    
                    fig_car.update_layout(height=350, margin=dict(t=10, b=10, l=10, r=10), template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5))
                    st.plotly_chart(fig_car, use_container_width=True)
                    st.caption("T=0(이벤트 시작일) 기준 누적 초과 궤적. 단기 반짝이 아닌 지속적 우상향(브랜드 인지도 제고) 입증.")

            with c_chart2:
                with st.container(border=True):
                    st.markdown("**📊 시장효과(Market) vs 설정효과(Setup) 자금 분해**")
                    # Mock Data for Market vs Setup Effect
                    weeks = ["W1 (코스피 급등기)", "W2 (코스피 보합기)"]
                    market_effect = [-15.0, 0.5] # W1: 차익실현 환매, W2: 시장효과 미미
                    setup_effect = [8.1, 33.5]   # 신규 유입
                    
                    fig_decomp = go.Figure(data=[
                        go.Bar(name='시장효과 (지수 등락 연동)', x=weeks, y=market_effect, marker_color='gray'),
                        go.Bar(name='순수 설정효과 (마케팅/영업 유입)', x=weeks, y=setup_effect, marker_color='#ff4d4d')
                    ])
                    ...
                    st.caption("코스피 변동분을 제거한 순수 마케팅 기반 신규 자금(설정효과)을 분리하여 시각화합니다.")

            st.divider()

            # [Step 4] 하단: 검색량-순매수 시차 상관관계 분석기 (Time-Lag Cross Correlation)
            st.markdown("#### ⏱️ [Step 4] 검색량-순매수 시차 상관관계 분석 (Time-Lag Analysis)")
            st.info("💡 **구조적 배경:** 은행 채널은 신탁/퇴직연금 경유 및 PB 영업 프로세스의 특성상 즉각적인 충동 매수가 차단되므로, 마케팅 효과가 나타나기까지 수주의 시차가 발생합니다. 아래 분석기에서 상관계수가 가장 높은 주차가 마케팅 효과의 **'최적 도달 시차'**입니다.")
            
            with st.container(border=True):
                # Mock Data for Time-Lag Cross Correlation
                lags = ["-2주", "-1주", "0주 (당일)", "+1주", "+2주", "+3주", "+4주"]
                correlations = [0.15, 0.22, 0.35, 0.52, 0.68, 0.85, 0.71]
                colors = ['gray'] * 5 + ['#4da6ff'] + ['gray'] # +3 weeks highlight
                
                fig_lag = go.Figure(data=[
                    go.Bar(x=lags, y=correlations, marker_color=colors, text=[f"{c:.2f}" for c in correlations], textposition='auto')
                ])
                
                # Add annotation for the peak
                fig_lag.add_annotation(x="+3주", y=0.85, text="r=0.85 (강한 정상관 도달)", showarrow=True, arrowhead=1, arrowcolor="#ffb04d", font=dict(color="#ffb04d", size=13), yshift=10)
            ...
            with st.container(border=True):
                ...
                <div style='padding:15px; background:rgba(77, 166, 255, 0.05); border-radius:10px; border:1px solid rgba(77, 166, 255, 0.2);'>
                    <ul style='margin-bottom: 0;'>
                        <li><b>DiD 성과:</b> 대조군({comp_etf}) 대비 타겟({target_etf})의 순매수 유입이 <b>{mock_did_multiplier}배</b> 초과 달성되었습니다.</li>
                        <li><b>통계적 유의성:</b> 이벤트 기간 내 브랜드 검색량 증가(+{mock_brand_search_inc}%)는 통계적으로 매우 유의미합니다 (p-value: {mock_p_value:.4f}).</li>
                        <li><b>시차 효과:</b> 마케팅 발동 후 은행 채널의 구조적 지연으로 인해 <b>+3주 시점</b>에 순매수와의 상관계수가 극대화(r=0.85)되는 현상이 입증되었습니다.</li>
                    </ul>
                    <p style='margin-top: 10px; font-size: 13px; color: #94a3b8;'>※ 해당 수치들은 상단 '🤖 AI 프롬프트' 탭의 리포트 생성 체인에 자동으로 주입됩니다.</p>
                </div>
                """
            ...
            if len(sandbox_tickers) > 0:
                with st.spinner("해외 API에서 실제 주가 데이터를 수집하여 Total Return 지수를 합성하고 있습니다..."):
                    ...
                    if len(df_list) > 0:
                        ...
                        fig_fan.add_trace(go.Scatter(x=dates, y=bm_cum_returns, mode='lines', name='S&P 500 (TR 가상)', line=dict(color='gray', width=1, dash='dot')))
    ...
    # -------------------------------------------------------------------------
        ...
        with prompt_tabs[0]:
            ...
            **🤖 [핵심 Tip] 멀티모달(Vision) AI 200% 활용하기**
            ...
            추가적으로 **각 Step 안내에 적힌 위치로 이동해 차트 이미지를 다운받아 함께 첨부**하시면 AI가 훨씬 더 정교하게 시각적 추세를 분석해 줍니다.
            """)
...
[AUM 현황 데이터]: {st.session_state.aum_context_text}"""
            ...
            st.caption("📸 **권장 첨부 이미지:** [ETF 시장 모니터링] ➡️ [📈 순매수 & 수익률] 탭의 **'주간 수익률 vs 순매수 증감률 산점도'** (우측 상단 📷 아이콘 클릭)")
...
[경쟁사 미디어 동향]: {st.session_state.media_context}"""
            ...
            st.caption("📸 **권장 첨부 이미지:** [ETF 시장 모니터링] ➡️ [📺 경쟁사 이벤트/동향] 탭의 **'마케팅 촉매 임팩트 분석기 (선 그래프)'** (우측 상단 📷 아이콘 클릭)")
            p1_step3 = """[Step 3: 최종 주간 마케팅 리포트 산출]
Step 1과 Step 2의 분석 결과를 종합하여, 리테일 마케팅 본부장에게 보고할 '주간 세일즈 액션 플랜 리포트'를 마크다운 형식으로 작성하시오. 리포트에는 반드시 다음 주 추천 셀링 포인트(Selling Point) 3가지가 포함되어야 함."""
