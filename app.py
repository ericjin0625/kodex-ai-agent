...
# =========================================================================
        ...
        # [수정] AI 프롬프트 탭을 통폐합하여 4번째 하위 탭으로 편입
        ...
        with sub_tabs_plan[0]:
            ...
            # [수정] 뉴스 검색 확률을 높이기 위한 느슨한 키워드 매핑 세팅
            search_kw_map = {
            ...
            with col_mon2:
                ...
                with st.spinner("에프앤가이드 및 KRX 지수 발표 뉴스를 스크랩 중..."):
                    # [수정] 무지성 지수 뉴스가 아닌 타겟 테마의 지수만 핀포인트 스크래핑
                    index_query = f'({search_kw_map[selected_trend_label]}) AND ("에프앤가이드" OR "한국거래소" OR "지수 산출" OR "지수 발표")'
                ...
                with st.spinner("해외 API에서 실제 주가 데이터를 수집하여 Total Return 지수를 합성하고 있습니다..."):
                    ...
                    if len(df_list) > 0:
                        ...
                        if not api_error:
                            ...
                            # [수정] 텍스트 겹침 현상 방지: 내부 Title 삭제 후 외부 마크다운으로 제목 분리
        ...
        # [수정] 메인 탭 통폐합에 따른 AI 상품기획 프롬프트 편입 (4번째 하위 탭)
        with sub_tabs_plan[3]:
            st.markdown("### 🤖 글로벌 대체자산 ETF 상품기획 마스터 프롬프트 (5-Step)")
