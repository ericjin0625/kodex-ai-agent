...
- 지시사항: {csv_directive} 이 스크리닝 룰과 편입 후보 종목들을 기반으로 지수 산출 방법론을 구체화하고, 가중치 배분([{st.session_state.p_weight}], 단일 종목 상한 {st.session_state.p_cap}%) 룰 및 S&P DJI 방식의 리밸런싱/이벤트 처리 원칙을 명시할 것."""
            ...
            # [수정] Step 3: 상관계수 및 추적오차(TE) 반영
            st.markdown("**📌 [Step 3: 퀀트 성과 및 리스크 검증]**")
            st.caption("📸 **권장 첨부 이미지:** [글로벌 상품 기획 시뮬레이터] ➡️ [💡 3. 가상 지수 샌드박스] 탭의 **'하이브리드 시나리오 차트 (밴드 차트)'** (우측 상단 📷 아이콘 클릭)")
...
- 지시사항: 1) S&P 500과의 낮은 상관계수를 근거로 기존 전통자산 포트폴리오 편입 시 분산(헷지) 효과를 수학적으로 증명할 것. 2) 총수익률을 자본차익과 인컴수익으로 철저히 분해하여, 하락장 방어 논리를 어필할 것. 3) 과거 매크로 위기 국면([{st.session_state.p_scenario}]) 당시의 벤치마크 대비 방어력을 분석할 것. 4) 합성 지수의 복제 추적오차(Tracking Error) 리스크를 통제하기 위한 실제 운용 매니저 관점의 대응 전략을 추가할 것."""
            st.code(p2_step3, language="text")

            # [수정] Step 4: 구조화 및 P&L 타당성 분석
...
- 지시사항: 1) 본 상품의 환율 전략과 인컴 속성을 고려할 때, 고객 세후 수익률에 유리한 연금/ISA 채널 타겟팅 전략을 세울 것. 2) 위 P&L 데이터를 근거로, 상품 런칭 시점의 초기 시딩(Seeding) 규모와 마케팅 비용 집행이 시장 점유율 확보 관점에서 왜 타당한 투자인지 강력하게 설득할 것."""
            st.code(p2_step4, language="text")

            # [수정] Step 5: 최종 보고서 및 팩트시트
